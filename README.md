...
| Conformance bench — Slice 0.3 | **dialect swap — the XML deficit was a spec/prior mismatch, and in-band data leaks syntax into control** | [`bench/FINDINGS-SLICE0.3.md`](bench/FINDINGS-SLICE0.3.md) |
...
- **Prompt-embedded envelopes leak syntax from data into control.** With the dialect matched, every dialect violation landed on the one task whose file content contains the *other* dialect, and on none of the others: a model was handed a payload as data, and its next tool call adopted that payload's syntax. The parse succeeded and the task succeeded — only the syntax drifted. A native schema cannot do this, because the call never shares a channel with the content. n=2; the falsification is stated in [`bench/FINDINGS-SLICE0.3.md`](bench/FINDINGS-SLICE0.3.md).
